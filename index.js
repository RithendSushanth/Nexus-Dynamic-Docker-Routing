...
// MongoDB connection
mongoose.connect('mongodb://127.0.0.1:27017/reverse-proxy')
    ...
    try {
      ...
      if (event.Type === 'container' && (event.Action === 'start' || event.Action === 'die')) {
        ...
        try {
          ...
          console.log(`${event.Action === 'start' ? 'Registering' : 'Updating'} container ${containerName}.localhost with IP ${ipAddress} and default port ${defaultPort} ---> ${target}`);
          ...
          await Container.findOneAndUpdate(
            ...
            { 
              ...
              defaultPort, 
              target,
              image: containerInfo.Config.Image.split(':')[0],
              tag: containerInfo.Config.Image.split(':')[1] || 'latest',
              status
            },
...
app.use(async (req, res, next) => {
  ...
  const container = await Container.findOne({ containerName: subdomain });

  if (!container) {
    return next(); // Pass to the next middleware if no container found
  }
...
app.get('/api/container/:containerName', async (req, res) => {
  try {
    const container = await Container.findOne({ containerName: req.params.containerName });
    if (!container) {
      return res.status(404).json({
        status: 'error',
        message: 'Container not found',
